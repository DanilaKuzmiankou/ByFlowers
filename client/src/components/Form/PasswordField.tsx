...
export const PasswordField: FC<PasswordFieldProps> = ({
  ...
  <>
    <div
      className={`${
        isPasswordFieldTouched && passwordFieldErrors ? 'error-icon' : null
    ...
    >
      ...
      <Field
        name="password"
        className={`${
          isPasswordFieldTouched && passwordFieldErrors ? 'error-field' : null
        } formik-field`}
      >
    ...
    {showStrengthBar && isPasswordFieldTouched && !passwordFieldErrors ? (
      <div style={{ width: '100%' }}>
        <PasswordStrengthBar
          password={password}
          minLength={8}
          barColors={['#aca9a9', '#ef4836', '#f6b44d', '#2b90ef', '#25c281']}
        />
      </div>
    ) : null}
    <ErrorMessage
